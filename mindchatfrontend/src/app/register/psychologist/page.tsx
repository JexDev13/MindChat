...
export default function RegisterPsychologistPage() {
 const [form, setForm] = useState({ firstName: "", lastName: "", email: "", professionalLicense: "", university: "", graduationDate: "", password: "", confirmPassword: "" });
 ...
 body.set("ProfessionalLicense", form.professionalLicense);
 body.set("University", form.university);
 body.set("GraduationDate", form.graduationDate);
 body.set("Password", form.password);
 body.set("ConfirmPassword", form.confirmPassword);
 body.set("__RequestVerificationToken", antiForgeryToken);
 const resp = await axios.post("/backend/Auth/RegisterPsychologist", body, { headers: { "Content-Type": "application/x-www-form-urlencoded", "X-XSRF-TOKEN": antiForgeryToken }, withCredentials: true, maxRedirects:0, validateStatus: s => s >=200 && s <400 });
 if (resp.status ===200) {
 ...
 <div className="w-full max-w-2xl glass p-8 border border-white/10 bg-white/5">
 <h1 className="text-2xl font-semibold mb-6">Crear cuenta psicologo</h1>
 ...
 <div>
 <label className="block text-sm text-zinc-300">Licencia Profesional</label>
 <input value={form.professionalLicense} onChange={e=>update("professionalLicense", e.target.value)} className="w-full mt-1 bg-black/40 border border-white/10 rounded px-3 py-2" required />
 </div>
 <div>
 <label className="block text-sm text-zinc-300">Universidad</label>
 <input value={form.university} onChange={e=>update("university", e.target.value)} className="w-full mt-1 bg-black/40 border border-white/10 rounded px-3 py-2" required />
 </div>
 <div>
 <label className="block text-sm text-zinc-300">Ano de Graduacion</label>
 <input type="date" value={form.graduationDate} onChange={e=>update("graduationDate", e.target.value)} className="w-full mt-1 bg-black/40 border border-white/10 rounded px-3 py-2" required />
 </div>
 <div>
